...
- [x] 验证 OpenAI-compatible endpoint 基础请求。
...
- [x] 评估 OpenAI-only 镜像是否仍需 Node.js 和 Claude CLI。
...
| 2026-08-31 | OpenAI-only 镜像依赖边界 | Dockerfile 增加 `openai-runtime` Python-only target；默认 `full-runtime` 继续保留 Node 22、Claude CLI 和 npm 云效 MCP server；`docker build --target openai-runtime` 与容器内 `python cli.py --help` 通过，`node`/`claude` 不存在 |
